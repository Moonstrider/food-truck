...
import com.mountains.micro.common.exception.SystemException;
...
public class DateUtil {
    ...
    public static String localDateTimeToString(LocalDateTime dateTime, String pattern) {
        if (ValidationUtil.isObjectNull(dateTime)) {
            return null;
        }
    ...
    public static LocalDateTime stringToLocalDateTime(String dateTime, String pattern) {
        if (ValidationUtil.isStringEmpty(dateTime)) {
            return null;
        }
    ...
    public static String dateToString(Date date, String pattern) {
        if (ValidationUtil.isObjectNull(date)) {
            return null;
        }
    ...
    public static String convertUTCtoTimeZone(Date date, String timeZone) {
        if (date == null) {
            return null;
        }
        String utcDateString = dateToString(date, DatePattern.YYYY_MM_DD_T_HH_MM_SS_S_Z);

        SimpleDateFormat utcFormat = new SimpleDateFormat(DatePattern.YYYY_MM_DD_T_HH_MM_SS_S_Z);
        utcFormat.setTimeZone(TimeZone.getTimeZone("UTC"));
        SimpleDateFormat timeZoneFormat = new SimpleDateFormat(DatePattern.YYYY_MM_DD_HH_MM_SS);
        timeZoneFormat.setTimeZone(TimeZone.getTimeZone(timeZone));

        try {
            return timeZoneFormat.format(utcFormat.parse(utcDateString));
        } catch (ParseException e) {
            log.error("Can't parse date [{}]", utcDateString, e);
    ...
    public static Date getInsDateWithoutTime(int ins) {
        Date today = getCurrentJavaDate();
        return getInsDateWithoutTime(today, ins);
    }
    ...
    public static Date getLastSecondDate(Date date) {
        ...
        return stringToDate(newDateSting, DatePattern.YYYY_MM_DD_HH_MM_SS);
    }
    ...
    public static Date getFirstDayOfFirstWeekOfYear(Integer year) {
        Date firstDayOfYear = stringToDate(year.toString() + "-01-01", DatePattern.YYYY_MM_DD);
        Calendar cal = Calendar.getInstance();
        ...
        if (ValidationUtil.isStringNotEmpty(firstDayString)) {
            return stringToDate(firstDayString, DatePattern.YYYY_MM_DD);
        }
    ...
    public static Integer getYMByDate(Date date) {
        return Integer.valueOf(dateToString(date, DatePattern.YYYYMM));
    }

    public static Date getFirstDayOfMonth(Integer ym) {
        return stringToDate(ym.toString() + "01", DatePattern.YYYYMMDD);
    }
